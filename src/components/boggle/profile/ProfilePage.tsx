...
export const ProfilePage = component$(() => {
  ...
  return (
    <div data-testid="profile-page" style="max-width: 920px; margin: 0 auto; padding: 32px 24px 64px;">
      {/* Header */}
      <header style="display: flex; align-items: center; gap: 18px; margin-bottom: 28px;">
        <Avatar playerId={playerId} displayName={me?.displayName ?? ''} size={72} showOnlineDot />
        <div style="display: flex; flex-direction: column; gap: 4px; flex: 1; min-width: 0;">
          ...
          <span style="font-size: 11px; color: #94a3b8; font-family: ui-monospace, monospace;">
            {playerId || 'loading…'}
          </span>
        </div>
      ...
      {/* Tabs */}
      <nav role="tablist" style="display: flex; gap: 2px; border-bottom: 1px solid #e2e8f0; margin-bottom: 24px;">
        {TABS.map((t) => {
          ...
          return (
            <button
              ...
              style={`padding: 10px 16px; background: transparent; border: 0; cursor: pointer; font-size: 13px; font-weight: ${active ? 600 : 500}; color: ${active ? '#0f172a' : '#64748b'}; border-bottom: 2px solid ${active ? '#f59e0b' : 'transparent'}; margin-bottom: -1px; display: flex; align-items: center; gap: 6px; transition: color 0.12s, border-color 0.12s;`}
            >
...
export const OverviewTab = component$(() => {
  ...
  return (
    <div data-testid="profile-tab-body-overview" style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
